...
function getAdjustedMouseCoordinates(mouseX, mouseY) {
  const adjustedX =
    (mouseX - GenealogicalTree.panOffset.x + GenealogicalTree.scaledOffset.x) /
    GenealogicalTree.scale;
  const adjustedY =
    (mouseY - GenealogicalTree.panOffset.y + GenealogicalTree.scaledOffset.y) /
    GenealogicalTree.scale;
...
export function monitorInteraction() {
  GenealogicalTree.canvas.addEventListener("pointermove", (e) => {
    if (e.pressure > 0.2) {
      const [adjustedX, adjustedY] = getAdjustedMouseCoordinates(
        e.offsetX,
        e.offsetY
      );
